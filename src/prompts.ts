...
export function buildCliPrompt(
  ...
  const discussionsSection = sampledDiscussions.length
    ? (lang === "en"
        ...
        : `\n## 最新 Discussions（过去 7 天内更新）${sampleNote(discussions.length, sampledDiscussions.length, lang, "engagement")}\n`) +
...
1. **This Week's Highlights** - 2-3 sentences summarizing the most important updates
...
## 最新 Issues（过去 7 天内更新）${issueNote}
${issuesText}

## 最新 Pull Requests（过去 7 天内更新）${prNote}
${prsText}
${discussionsSection}
---
...
5. **Stability & Regressions** - Crashes, correctness bugs, regressions reported this week, ranked by severity, note if fix PRs exist
...
5. **稳定性与回归** - 本周报告的崩溃、正确性 Bug、回归问题，按严重程度排列，标注是否已有 fix PR
...
export function buildInfraComparisonPrompt(
  ...
  const noActivityStr = lang === "en" ? "No activity in the last 7 days." : "过去 7 天无活动。";
...
1. **Ecosystem Overview** - 3-5 sentences on the overall AI infrastructure landscape this week
...
6. **Trend Signals** - Industry trends extracted from this week's activity, and what agent/application developers should watch
...
2. **各项目活跃度对比** - 以表格形式汇总各项目本周的 Issues 数、PR 数、Release 情况
...
6. **值得关注的趋势信号** - 从本周动态中提炼行业趋势，以及 Agent/应用开发者应当关注什么
...
1. **This Week's Overview** - 3-5 sentences summarizing project status, including activity assessment
2. **Releases** - If new versions exist, detail changes, breaking changes, migration notes; omit if none
3. **Project Progress** - Merged/closed PRs this week, what features advanced or were fixed
4. **Community Hot Topics** - Most active Issues/PRs with most comments/reactions (with links), analyze underlying needs
5. **Bugs & Stability** - Bugs, crashes, regressions reported this week, ranked by severity, note if fix PRs exist
...
# 数据概览
- 过去 7 天 Issues 更新：${totalIssues} 条（新开/活跃: ${openIssues}，已关闭: ${closedIssues}）
- 过去 7 天 PR 更新：${totalPrs} 条（待合并: ${openPrs}，已合并/关闭: ${mergedPrs}）
...
1. **本周速览** - 用3-5句话概括项目本周整体状态，包括活跃度评估
2. **版本发布** - 如有新版本，详细说明更新内容、破坏性变更、迁移注意事项；无则省略
3. **项目进展** - 本周合并/关闭的重要 PR，说明推进了哪些功能或修复，项目整体向前迈进了多少
4. **社区热点** - 本周讨论最活跃、评论最多、反应最多的 Issues/PRs（附链接），分析背后的诉求
5. **Bug 与稳定性** - 本周报告的 Bug、崩溃、回归问题，按严重程度排列，标注是否已有 fix PR
...
2. **各项目活跃度对比** - 以表格形式汇总各项目本周的 Issues 数、PR 数、Release 情况及健康度评估
...
2. **Activity Comparison** - Table comparing Issues count, PR count, Discussions count and Release status for each tool this week. Some repos have Issues/PRs disabled upstream and use Discussions as their only community channel — mark those as "N/A" rather than reporting them as inactive
...
2. **各工具活跃度对比** - 以表格形式汇总各工具本周的 Issues 数、PR 数、Discussions 数、Release 情况。部分仓库上游关闭了 Issues/PR，仅以 Discussions 作为社区渠道，这类请标注 "N/A"，不要判定为不活跃
