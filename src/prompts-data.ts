...
import { trendingRangeLabel } from "./window.ts";
      ...
      ? data.trendingRepos
          .map(
            (r) =>
              ...
              (r.periodStars > 0 ? ` (+${r.periodStars} ${trendingRangeLabel()})` : "") +
              (r.forks > 0 ? ` 🍴${r.forks.toLocaleString()}` : "") +
              (r.description ? `\n  ${r.description}` : ""),
          )
          .join("\n")
      : lang === "en"
        ? "(Unable to fetch this week's GitHub Trending list)"
        : "（未能抓取本周 GitHub Trending 榜单）";
...
## Data Sources
- **Trending List** (github.com/trending, this week's stars most reliable): Real-time hot list with this week's new stars
- **Topic Search** (GitHub Search API, topic tags): AI-related projects active in last 7 days, grouped by topic

---

## GitHub This Week's Trending (${data.trendingRepos.length} repositories)
${trendingSection}
...
1. **This Week's Highlights** — 3-5 sentences on the most noteworthy AI open-source developments this week
...
2. **Top Projects by Category** — For each category, render a **Markdown table** with exactly these columns:
   ...
   - **Stars**: total stars, plus this week's new stars in parentheses when available (e.g. "86,392 (+1,851)"); copy the numbers from the input verbatim, do not recompute
   - **Summary**: 2 sentences — what the project is and why it's worth attention this week, including any standout data point or momentum signal
   - List 3-8 projects per category; omit a category's table entirely if no project falls under it

3. **Trend Signal Analysis** — 200-300 words, distill from this week's hot list:
...
## 数据说明
- **Trending 榜单**（github.com/trending，本周 stars 数最可信）：本周实时热榜，含本周新增 stars
- **主题搜索**（GitHub Search API，topic 标签）：7天内活跃的 AI 相关项目，按主题分类

---

## GitHub 本周 Trending 榜单（共 ${data.trendingRepos.length} 个仓库）
${trendingSection}
...
1. **本周速览** — 3~5 句话概括本周 AI 开源领域最值得关注的动向

2. **各维度热门项目** — 每个维度用 **Markdown 表格**呈现，列固定为：

   | 项目 | 语言 | Stars（总量 / 本周） | 简要说明 |
   | :--- | :--- | ---: | :--- |

   - **项目**：仓库名，做成指向其 GitHub 链接的 Markdown 链接
   - **语言**：主要语言（未知则留空）
   - **Stars**：总 star 数，有本周新增则在括号中标注（如 "86,392（+1,851）"）；数字照抄输入，不要重算
   - **简要说明**：2 句话——项目是什么、本周为什么值得关注，点出关键数据或增长信号
   - 每个维度列 3~8 个项目；某维度下若无项目则整张表省略

3. **趋势信号分析** — 200~300 字，从本周热榜中提炼：
   - 哪类 AI 工具正在获得社区爆发性关注？
   - 有无新兴技术栈或方向首次登榜？
   - 与近期大模型发布/行业事件的关联
      ...
      const mode =
        lang === "en"
          ? isFirstRun
            ...
            : `Incremental update, ${newItems.length} new articles this week`
          : isFirstRun
            ? `首次全量抓取（sitemap 共 ${totalDiscovered} 条 URL，以下为最新 ${newItems.length} 篇正文内容）`
            : `本周增量更新，共 ${newItems.length} 篇新内容`;
  ...
  const firstRunNote =
    lang === "en"
      ? isAnyFirstRun
        ...
        : "This is an incremental update. Please focus on this week's new content and assess its strategic significance in context."
      : isAnyFirstRun
        ? "本次为首次全量抓取，请重点梳理各站点的内容格局、历史脉络与核心主题，而非仅关注单篇文章。"
        : "本次为增量更新，请聚焦本周新增内容，并结合上下文判断其战略意义。";
...
1. **This Week's Highlights** — 3-5 sentences on the most important new releases or developments, calling out key highlights
...
1. **本周速览** — 3~5 句话概括最重要的新发布或动向，点出核心亮点
...
export function buildHighlightsPrompt(
  ...
  if (lang === "en") {
    return `You are a concise news editor. The following are this week's AI ecosystem report excerpts, each labeled with a report ID.
...
- Be specific: include project names, version numbers, star counts where relevant`;
  ...
  return `你是一位简洁的新闻编辑。以下是本周 AI 生态各报告的摘要，每个报告用 ID 标注。
...
1. **This Week's Highlights** — 3-5 sentences on the hottest AI discussion topics and community sentiment on HN this week
...
3. **Community Sentiment Signal** — 100-200 words analyzing this week's HN AI discussion mood and focus:
...
1. **本周速览** — 3~5 句话，概括本周 HN 社区围绕 AI 最热门的讨论方向和情绪
...
3. **社区情绪信号** — 100~200 字，分析本周 HN AI 讨论的整体情绪和关注重点：
   - 社区对哪类话题最活跃（高分 + 高评论）？
   - 有无明显的争议点或共识？
   - 与上周期相比，关注方向有无明显变化？

4. **值得深读** — 列出 2~3 条本周最值得开发者/研究者深入阅读的内容，简述理由
...
export function buildPhPrompt(data: PhData, dateStr: string, lang: Lang = "zh"): string {
  ...
  if (lang === "en") {
    return `You are an AI product analyst. The following are AI-related products launched on Product Hunt in the past 7 days as of ${dateStr} (sorted by votes, ${data.products.length} total):
...
1. **This Week's Highlights** — 3-5 sentences on the most notable AI product launches and trends on Product Hunt this week
...
3. **Market Signal** — 100-200 words analyzing this week's Product Hunt AI launch patterns:
...
`;
  ...
  return `你是 AI 产品分析师。以下是 ${dateStr} 从 Product Hunt 抓取的过去 7 天内 AI 相关产品发布（按投票数降序，共 ${data.products.length} 个）：
...
1. **本周速览** — 3~5 句话，概括本周 Product Hunt 上 AI 产品发布的整体趋势和亮点
...
3. **市场信号** — 100~200 字，分析本周 Product Hunt AI 产品的发布规律：
   - 哪些类别最密集？
   - 有无创新性的思路或新颖的应用场景？
   - 开源 vs 闭源的趋势
...
1. **This Week's Highlights** — 3-5 sentences on the most significant research directions and breakthroughs
...
3. **Research Trend Signal** — 100-200 words on emerging research directions visible from this week's submissions
...
1. **本周速览** — 3~5 句话，概括本周最值得关注的研究方向和突破
...
3. **研究趋势信号** — 100~200 字，从本周投稿中观察到的新兴研究方向
...
1. **This Week's Highlights** — 3-5 sentences on the most notable model releases and trends on Hugging Face
...
1. **本周速览** — 3~5 句话，概括 Hugging Face 上最值得关注的模型发布和趋势
...
1. **This Week's Highlights** — 3-5 sentences on the most discussed AI topics across these communities this week
...
1. **本周速览** — 3~5 句话，概括本周技术社区围绕 AI 最热门的讨论方向
